...
For Hermes Infra team members and automation agents.
...
For open-source contributors outside Hermes Infra.
...
```bash
git clone git@github.com:YOUR_USERNAME/hermes-infra.git
cd hermes-infra
```
...
```bash
git remote add upstream git@github.com:0xPlayerOne/hermes-infra.git
git fetch upstream
...
_Maintainers: Hermes Infra engineering team_
