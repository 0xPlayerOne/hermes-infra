...
SKIP_REPOS = (
    set(os.environ.get("SKIP_REPOS", "").split(",")) if os.environ.get("SKIP_REPOS") else set()
)
...
def restart_tei():
    ...
    try:
        ...
        subprocess.run(["pkill", "-9", "-f", "text-embeddings"], capture_output=True, timeout=10)
        time.sleep(2)
        # unload + reload to force a clean respawn
        subprocess.run(["launchctl", "unload", TEI_PLIST], capture_output=True, timeout=10)
        time.sleep(1)
        subprocess.run(["launchctl", "load", TEI_PLIST], capture_output=True, timeout=10)
...
def cmd_index(reindex=False):
    ...
    col = chroma.get_or_create_collection(
        name="code",
        metadata={"model": MODEL, "dev_root": str(DEV_ROOT)},
    ...
    for repo_path, repo_name in repos:
        ...
        if _too_big or (MAX_REPO_FILES and len(candidates) > MAX_REPO_FILES):
            log(
                f"  ⚠️ {repo_name}: {len(candidates)}+ files — TOO LARGE, skipping (raise MAX_REPO_FILES to include)"
            )
            continue
        ...
        for p in candidates:
            ...
            try:
                ...
                if (
                    p.suffix.lower() in (".json", ".csv", ".sql", ".parquet")
        ...
        if repo_ids:
            ...
            for b0 in range(0, len(repo_ids), BATCH):
                ...
                if not ok:
                    ...
                    for rel in set(repo_ids[i].split("#")[0].split("::")[1] for i in range(b0, b1)):
        ...
        save_state(
            state
        )  # CHECKPOINT: persist after each repo so interruptions don't lose progress
...
if __name__ == "__main__":
    ...
    if args.index:
        ...
        cmd_index(reindex=True)
    elif args.status:
        cmd_status()
    elif args.query:
        cmd_query(args.query, args.repo, args.n)
    else:
