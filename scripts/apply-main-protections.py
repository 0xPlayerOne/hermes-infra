...
REPOS = [
    ...
    ("nifty-contracts-api",   "NiftyLeague/nifty-contracts-api",
     ["Build, Format, Lint & Type Check", "CI / Test", "Review dependency changes", "Vercel Preview Comments"]),
...
def apply_protection(repo, remote, checks):
    ...
    result = subprocess.run(
        ["gh", "api", f"repos/{remote}/branches/main/protection",
         "--method", "PUT",
         "-H", "Content-Type: application/json",
         "-f", f"payload={json.dumps(payload)}"],
        capture_output=True, text=True, timeout=30, cwd=CWD
    )

    if result.returncode == 0:
        print(f"  ✅ Main protection applied")
    else:
    ...
    result2 = subprocess.run(
        ["gh", "api", f"repos/{remote}/merge-queue",
         "--method", "PUT",
         "-H", "Content-Type: application/json",
         "-f", f"payload={json.dumps(mq_payload)}"],
        capture_output=True, text=True, timeout=30, cwd=CWD
    )

    if result2.returncode == 0:
        print(f"  ✅ Merge queue configured (optional)")
    else:
