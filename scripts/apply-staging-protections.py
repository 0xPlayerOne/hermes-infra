...
for name, remote in REPOS:
    print(f"\n--- {name} ({remote}) ---")
    r = subprocess.run(
        ["gh", "api", f"repos/{remote}/branches/staging/protection",
         "--method", "PUT", "--input", "/tmp/staging-protection.json"],
        capture_output=True, text=True, timeout=30, cwd=CWD
    )
    if r.returncode == 0:
        print(f"  ✅ Staging protection applied")
    else:
        err = r.stderr.strip()
        if "Upgrade to GitHub Pro" in err or "403" in err:
            print(f"  ⚠️  GitHub Free limitation (private repo) - cannot set via API")
        else:
