...
def test_export_memory_facts_and_bad_json(load_script, tmp_path, monkeypatch):
    ...
    memory.mkdir()
    (memory / "facts.json").write_text(json.dumps([
        {"content": "first", "target": "memory"}, {"memory": "second"}
    ]), encoding="utf-8")
    (memory / "bad.json").write_text("{bad", encoding="utf-8")
    module.MEM_FACTS_DIR = str(tmp_path / "out")
    original = module.os.path.expanduser
    monkeypatch.setattr(module.os.path, "expanduser", lambda path: str(memory) if path == "~/.hermes/memory" else original(path))
...
def test_synthesis_recent_messages(load_script, tmp_path):
    ...
    con.execute("INSERT INTO messages VALUES ('user', ?, ?)", (9999999999, "A sufficiently long user message"))
    con.execute("INSERT INTO messages VALUES ('assistant', ?, ?)", (9999999999, "ignored assistant message"))
    con.commit()
    con.close()
...
def test_queue_hindsight_paths(load_script, monkeypatch):
    ...
    monkeypatch.setattr(module.urllib.request, "urlopen", lambda *a, **k: (_ for _ in ()).throw(OSError()))
...
def test_synthesis_main_and_missing_db(load_script, tmp_path, monkeypatch, capsys):
    ...
    module.MEM_FACTS_DIR = str(tmp_path / "facts")
    module.PATTERNS = [(pattern, formatter, str(user))
                       for pattern, formatter, target in module.PATTERNS]
    monkeypatch.setattr(module, "get_recent_user_messages", lambda: ["I prefer Rust for local infrastructure."])
